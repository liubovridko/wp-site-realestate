...
$possible_plugin_locations = array(
	...
	'C:\xampp\htdocs\real-estate\wp-content\plugins\wp-optimize\cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://real-estate.test/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'C:\xampp\htdocs\real-estate\wp-content\plugins\wp-optimize\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-real-estate.test.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-real-estate.test.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-real-estate.test.php');
}
